...
class StartRecordingDialog extends AbstractStartRecordingDialog {
    ...
    isStartRecordingDisabled() {
        ...
        console.log('StartRecordingDialog: isStartRecordingDisabled check', {
            shouldRecordAudioAndVideo,
            shouldRecordTranscription,
            selectedRecordingService,
            isTokenValid
        ...
        if (!shouldRecordAudioAndVideo && !shouldRecordTranscription) {
            console.log('StartRecordingDialog: Button disabled - no audio/video or transcription selected');
            return true;
        }
        ...
        if (selectedRecordingService === RECORDING_TYPES.JITSI_REC_SERVICE) {
            console.log('StartRecordingDialog: Button enabled - JITSI_REC_SERVICE');
            return false;
        } else if (selectedRecordingService === RECORDING_TYPES.DROPBOX) {
            const disabled = !isTokenValid;
            console.log('StartRecordingDialog: Button', disabled ? 'disabled' : 'enabled', '- DROPBOX, tokenValid:', isTokenValid);
            return disabled;
        } else if (selectedRecordingService === RECORDING_TYPES.LOCAL) {
            console.log('StartRecordingDialog: Button enabled - LOCAL recording');
            return false; // Local recording is always available when supported
        }

        console.log('StartRecordingDialog: Button disabled - unknown service:', selectedRecordingService);
        return true;
